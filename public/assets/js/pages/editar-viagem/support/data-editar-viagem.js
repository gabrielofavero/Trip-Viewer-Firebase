...
async function _updateTripImage(body){
    ...
    if (user) {
        try {
            ...
            const backendURL =
                host === 'localhost'
                    ? `http://localhost:5001/trip-viewer-tcc/us-central1/updateTripImage?token=${token}`
                    : `https://us-central1-trip-viewer-tcc.cloudfunctions.net/updateTripImage?token=${token}`;
