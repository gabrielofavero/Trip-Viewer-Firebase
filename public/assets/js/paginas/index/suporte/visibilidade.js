...
function _loadNotificationBar() {
	if (CURRENT_TRIPS.length > 0) {
		...
		if (CURRENT_TRIPS.length == 1) {
			getID("notification-text").innerHTML =
				`${translate("trip.current_single")}:<br>${CURRENT_TRIPS[0].titulo}`;
			...
			getID("notification-text").innerHTML =
				`${translate("trip.current_multi_1")}<br> ${translate("trip.current_multi_2")}"`;
...
function _applyNotificationBarColor() {
	getID("notification-bar").style.backgroundColor = _isOnDarkMode()
		? notificationBar.escuro
		: notificationBar.claro;
}
