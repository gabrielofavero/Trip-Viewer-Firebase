...
export function openTripDialog(tripId) {
	...
	// Dates
	getID("trip-dialog-dates").textContent = dateObjectToString(trip.start) + " – " + dateObjectToString(trip.end);
...
export function loadDestinationsTab() {
	...
	empty.style.display = "none";
	count.textContent = destinations.length + " " + (destinations.length === 1 ? translate("destination.document") : translate("destination.title"));
...
export function loadListsTab() {
	...
	empty.style.display = "none";
	count.textContent = listings.length + " " + (listings.length === 1 ? translate("listing.document") : translate("listing.title"));
