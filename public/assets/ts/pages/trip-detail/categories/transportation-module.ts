...
function getSwiperData() {
	...
	ACTIVE_TRANSPORTATIONS = [
		...new Set(
			getState().transportation.data.map(
...
function getTransportationIcon(j) {
	...
	const icon =
		getTransportations().icons[type] || getTransportations().icons.other;
...
function getDurationHTML(j) {
	...
	if (!duration) return "";
		...
		return `<div class="flight-duration">${jsTimeToVisualTime(duration)}</div>`;
}
...
function autoNavigateTransportation() {
	...
	if (START_DATE?.date && END_DATE?.date) {
		if (
			today < getDateNoTime(START_DATE.date) ||
			today > getDateNoTime(END_DATE.date)
		) {
			targetIndex = 0;
		}
	}
	...
	if (targetIndex === undefined) {
		...
		for (let i = 0; i < data.length; i++) {
			const departure = getDateNoTime(
				convertFromDateObject(data[i].dates.departure),
			);
