...
export function addDestinationsListeners(category, j) {
	...
	// Emoji Validation
	getID(`${category}-emoji-${j}`).addEventListener("input", () =>
		emojisOnInputAction(j, category),
	);
...
export function updateDestinationsTitle(j, category) {
	...
	getID(`${category}-title-icon-${j}`).style.display = getID(
		`${category}-isNew-${j}`,
	).checked
		? "block"
		: "none";
...
export function openMoveDestinationModal(j, category) {
	...
	properties.title =
		...
		translate("destination.move.title", { category: firstCharToUpperCase(category) });
...
export function moveDestination(j, category) {
	...
	if (category != newCategory) {
		...
		updateValueDS(
			"region",
			destination.region,
			`${newCategory}-region-select-${newJ}`,
		);
		buildDS("region");
