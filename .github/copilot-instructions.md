...
- **🚫 NO AUTO-COMMIT / NO AUTO-DEPLOY.** Never run `git commit`, `git push`, `npm run sync`, `firebase deploy`, `wrangler deploy`, or any equivalent version-control/deployment command without the user's explicit authorization. Propose the exact command(s) and wait for approval before executing.
