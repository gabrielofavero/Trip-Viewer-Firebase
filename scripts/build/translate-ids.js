...
if (!dryRun && !writeMode) {
	console.error(
		"Usage: node scripts/build/translate-ids.js [--dry-run | --write]",
	);
	console.error(
		"  --dry-run   Log all planned changes without modifying files.",
	);
...
function processHTML(filePath) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const attrRegex =
			/\b(id|class|for|data-target|data-group|data-turno|data-categoria|data-periodo|data-translate)\s*=\s*"([^"]*)"/g;
		let match;
		...
		codePart = codePart.replace(
			...
			(fullMatch, className) => {
				...
				for (const [oldClass, newClass] of Object.entries(classMap)) {
					if (
						className.startsWith(oldClass + "-") ||
						className.startsWith(oldClass + "_")
					) {
		...
		for (const [oldDA, newDA] of Object.entries(dataAttrMap)) {
			...
			codePart = codePart.replace(dataRegex, (fullMatch, rest) => {
				...
				recordChange(
					filePath,
					lineNum,
					`[data-${oldDA}${rest.trimEnd()}`,
					`[data-${newDA}${rest.trimEnd()}`,
...
const DOM_FN_PATTERNS = [
	// getID("..."), getChildIDs("...")
	{
		regex: /\bgetID\s*\(\s*["'`]([^"'`]+)["'`]\s*\)/g,
		mapType: "id",
		fnName: "getID",
	},
	{
		regex: /\bgetChildIDs\s*\(\s*["'`]([^"'`]+)["'`]\s*\)/g,
		mapType: "id",
		fnName: "getChildIDs",
	},
	// document.getElementById("...")
	{
		regex: /\bgetElementById\s*\(\s*["'`]([^"'`]+)["'`]\s*\)/g,
		mapType: "id",
		fnName: "getElementById",
	},
	// querySelector("..."), querySelectorAll("...")  — could be id (#...) or class (....)
	{
		regex: /\bquerySelector(?:All)?\s*\(\s*["'`]([^"'`]+)["'`]\s*\)/g,
		mapType: "selector",
		fnName: "querySelector",
	},
	// classList.add("..."), classList.remove("..."), classList.toggle("..."), classList.contains("...")
	{
		regex:
			/\.classList\s*\.\s*(add|remove|toggle|contains)\s*\(\s*["'`]([^"'`]+)["'`]\s*\)/g,
		mapType: "class",
		fnName: "classList",
	},
	// element.matches("..."), element.closest("...")
	{
		regex: /\.(?:matches|closest)\s*\(\s*["'`]([^"'`]+)["'`]\s*\)/g,
		mapType: "selector",
		fnName: "matches/closest",
	},
	// dataset access: element.dataset.turno, dataset["turno"]
	{ regex: /\.dataset\s*\.\s*(\w+)/g, mapType: "dataAttr", fnName: "dataset" },
	{
		regex: /\.dataset\s*\[\s*["'`](\w+)["'`]\s*\]/g,
		mapType: "dataAttr",
		fnName: "dataset[]",
	},
];
...
function processTS(filePath) {
	...
	for (let i = 0; i < lines.length; i++) {
				...
				if (pattern.mapType === "id") {
					if (oldIds.has(argValue)) {
						...
						recordChange(
							filePath,
							lineNum,
							`${pattern.fnName}("${argValue}")`,
							`${pattern.fnName}("${newVal}")`,
							"TS DOM function (id)",
						);
					}
					...
					if (oldClasses.has(argValue)) {
						...
						recordChange(
							filePath,
							lineNum,
							`.classList.${fullMatch[1]}("${argValue}")`,
							`.classList.${fullMatch[1]}("${newVal}")`,
							"TS classList operation",
						);
					}
					...
					newSel = newSel.replace(/#([\w-]+)/g, (full, idPart) => {
						...
						for (const [oldId, newId] of Object.entries(idMap)) {
							if (
								idPart.startsWith(oldId + "-") ||
								idPart.startsWith(oldId + "_")
							) {
								selChanged = true;
					...
					newSel = newSel.replace(/\.([\w-]+)/g, (full, classPart) => {
						...
						for (const [oldClass, newClass] of Object.entries(classMap)) {
							if (
								classPart.startsWith(oldClass + "-") ||
								classPart.startsWith(oldClass + "_")
							) {
								selChanged = true;
		...
		while ((tMatch = templateRegex.exec(lines[i])) !== null) {
			...
			for (const rpl of allReplacements) {
				const overlaps = applied.some(
					(a) =>
						rpl.index < a.index + a.length && rpl.index + rpl.length > a.index,
				);
				if (!overlaps) {
					applied.push(rpl);
				}
		...
		const htmlAssignRegex =
			/\.(innerHTML|insertAdjacentHTML|outerHTML)\s*=\s*(`[^`]*`|"[^"]*"|'[^']*')/g;
		let haMatch;
		...
		while ((haMatch = htmlAssignRegex.exec(lines[i])) !== null) {
			...
			for (const [oldId, newId] of Object.entries(idMap)) {
				const idAttrRegex = new RegExp(
					`\\bid\\s*=\\s*["']${oldId.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`,
					"g",
				);
			...
			for (const [oldClass, newClass] of Object.entries(classMap)) {
				...
				const classWordRegex = new RegExp(
...
console.log("=".repeat(70));
console.log(
	`Mode: ${dryRun ? "DRY RUN (no files will be modified)" : "WRITE (files will be modified)"}`,
);
...
console.log(
	`Template prefix mappings: ${Object.keys(templatePrefixMap).length}`,
);
...
console.log("─".repeat(70));
console.log(
	"The following dynamic template literal patterns may need manual review:",
);
console.log("");
