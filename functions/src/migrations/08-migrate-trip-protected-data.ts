...
interface Hospedagem { id: string; reserva?: string; link?: string }
interface Transporte { id: string; reserva?: string; link?: string }
...
export const migrate = functions.https.onRequest(async (_req, res): Promise<void> => {
...
async function migrateViagens(batch: FirebaseFirestore.WriteBatch) {
    ...
    for (const doc of snap.docs) {
        ...
        if (data.pin == 'sensitive-only') {
            ...
            if (!pin) {
                throw new Error(`Trip ${doc.id} has gastosPin true but no pin found in gastos collection`)
            }
            const sensitive = getSensitiveTripObject(data)

            const protectedRef = admin.firestore()
                .collection("viagens")
                .doc("protected")
                .collection(pin)
                .doc(doc.id)
...
async function migrateGastos(batch: FirebaseFirestore.WriteBatch) {
    ...
    snap.forEach(doc => {
        ...
        if (pin) {
            const protectedRef = admin.firestore().collection("protegido").doc(doc.id)    
...
function getSensitiveTripObject(data: ViagemData) {
    ...
    for (const h of data.hospedagens ?? []) hospedagens[h.id] = { reserva: h.reserva, link: h.link }
    for (const t of data.transportes?.dados ?? []) transportes[t.id] = { reserva: t.reserva, link: t.link }

    if (!Object.keys(hospedagens).length && !Object.keys(transportes).length) return {}
