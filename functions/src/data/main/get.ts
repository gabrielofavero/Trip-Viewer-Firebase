...
export async function _getDataFromPath(
  path: string,
...
export function _getRefDataPath(
  refObject: interfaces.Referencia,
  response: functions.Response
) {
  if (
    !refObject ||
    ...
    response
      .status(400)
      .send("O objeto referenciado não possui a estrutura esperada.");
    return;
  }
...
export async function _getDataFromReference(
  refObject: interfaces.Referencia,
  response: functions.Response
) {
