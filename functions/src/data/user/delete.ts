...
export async function _deleteUser(
  ...
  try {
    ...
    for (let i = 0; i < viagensRefs.length; i++) {
      ...
      if (
        ...
        response
          .status(404)
          .send(
            `Viagem ${viagemPath} não encontrada ou não possui dados de hospedagem, programações ou transportes`
          );
        return;
      }
    ...
    response
      .status(200)
      .send(
        `Usuário e dados associados deletados com sucesso: ${JSON.stringify(
          deletedData
        )}`
      );
  } catch (error) {
    ...
    response
      .status(500)
      .send(`Erro ao deletar usuário: ${JSON.stringify(errorObj)}`);
  }
}
